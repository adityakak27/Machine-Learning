...
def helper_fn(disease):
    descr = description[description['Disease'] == disease]['Description']
    descr = " ".join([w for w in descr])

    pred = precautions[precautions['Disease'] == disease][['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']]
    pred = [col for col in pred.values]

    medic = medications[medications['Disease'] == disease]['Medication']
    medic = [med for med in medic.values]
    ...
    return descr, pred, medic, diet, work
...
def predict():
    if request.method == 'POST':
        ...
        description, prediction, medication, diet, workout = helper_fn(disease)

        return render_template('index.html', disease = disease, dis_desc = description, dis_pre = prediction, dis_med = medication, dis_diet = diet, dis_workout = workout)
